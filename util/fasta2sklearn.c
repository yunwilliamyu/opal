...
main (int argc, char *argv[])
{
	...
	if (status == GDL_SUCCESS)
	{
		...
		kseq_t *seq;
		FILE * stream = 0;
		size_t i, j, row, l, n, kmer_size, n_kmer_sizes, cptr, nlabels, tx_write, label_write, nkmers;
		size_t * label_tx, *kmer_sizes, *label_kmer;
		gdl_hashtable *taxid_to_label = 0;
		gdl_hashtable *kmer_to_label = 0;
		char taxid_name[20];
		FILE * taxid_stream  = 0;
		...
		*/
		...
		nlabels = 1;
		// hash table for kmer
		kmer_to_label = gdl_hashtable_alloc (gdl_interface_uint, 0);
		nkmers = 0;
				...
				if(seq->seq.l >= kmer_size){
					for(i = 0; i <= seq->seq.l - kmer_size; i++){
						...
						for(row = 0; row < numHash; row++){
							...
							// add to hash hash table (sanity check : check it does not exist already)
							if((label_kmer = (size_t*) gdl_hashtable_lookup (kmer_to_label, kmer)) == 0 ){
								label_kmer = GDL_CALLOC (size_t, 1);
								//new key
								*label_kmer = (size_t) (nkmers);
								nkmers++;
								// add 
								gdl_hashtable_add(kmer_to_label, kmer, label_kmer, 1);
							}
	   						// print foward kmer
							if(!stdout_flag){
								fprintf(stream, " %zu:", row);
								fprintf(stream, "%s:", kmer);
								fprintf(stream,"%zu", *label_kmer);								
							}else{
								printf(" %zu:", row);
								printf("%zu", *label_kmer);		
							}
